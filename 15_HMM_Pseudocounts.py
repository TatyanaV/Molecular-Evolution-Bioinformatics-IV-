...
CODE CHALLENGE: Solve the Profile HMM with Pseudocounts Problem.
     Input: A threshold θand a pseudocount , followed by an alphabet , followed by a
     multiple alignment Alignment whose strings are formed from .
     Output: The transition and emission matrices of HMM(Alignment, , σ).
...
with open('15_HMM_Pseudocounts.txt', "r") as f:
    text = f.read()
    lines = text.split('\n')
...
with open('15_HMM_Pseudocounts_Answer.txt', "w") as f:
    text = f.write(sol)